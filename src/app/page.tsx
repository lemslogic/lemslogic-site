...
export default function Home() {
  return (
    <main>
      ...
      <section className="section"><div className="shell"><div className="flex flex-col justify-between gap-6 md:flex-row md:items-end"><div><span className="tag">Projetos</span><h2 className="section-title mt-5">Soluções que já podem ser <span className="gradient-text">demonstradas.</span></h2></div><Link href="/projetos" className="btn-secondary project-link"><span>Ver projetos</span><span aria-hidden="true">↗</span></Link></div><div className="mt-12 grid gap-4 lg:grid-cols-3"><article className="card lg:col-span-2"><div className="flex flex-wrap items-center justify-between gap-3"><div className="text-sm text-[#55e694]">DEMO DISPONÍVEL</div><span className="rounded-full border border-[#55e694]/20 bg-[#55e694]/[.05] px-3 py-1 text-xs text-[#83f2b3]">ERP empresarial</span></div><h3 className="mt-14 text-3xl font-bold">LEMS Gestão</h3><p className="mt-4 max-w-2xl leading-7 text-white/55">ERP demonstrativo com gestão financeira, DRE Gerencial, fluxo de caixa, cadastros, relatórios e controle de acessos — desenvolvido para mostrar como processos empresariais podem virar software integrado.</p><div className="mt-8 flex flex-wrap gap-2">{["Next.js", "PostgreSQL", "Prisma", "Docker"].map((tech) => <span key={tech} className="rounded-full border border-white/10 px-3 py-1.5 text-xs text-white/45">{tech}</span>)}</div><Link href="/projetos#lems-gestao" className="mt-8 inline-flex items-center gap-2 text-sm font-semibold text-[#20d7df] transition hover:text-[#55e694]">Conhecer o projeto <span aria-hidden>↗</span></Link></article><article className="card"><div className="text-sm text-white/35">PRÓXIMOS CASES</div><div className="mt-16 space-y-4 text-lg"><p>Automação administrativa</p><p className="border-t border-white/10 pt-4">Estoque & pedidos</p><p className="border-t border-white/10 pt-4">Dashboards internos</p></div></article></div></div></section>
